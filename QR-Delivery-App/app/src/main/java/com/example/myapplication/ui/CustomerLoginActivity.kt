...
class CustomerLoginActivity : AppCompatActivity() {
    ...
    private fun attemptLogin(username: String, password: String) {
        LoginService.customerLogin(username, password) { deliveryman, msg ->
            runOnUiThread {
                if (deliveryman != null) {
                    println("登录成功：$deliveryman")
                    UserManager.getInstance(this@CustomerLoginActivity)?.saveUserCredentials(
                        deliveryman.id,
                        deliveryman.username,
                        deliveryman.password,
                        deliveryman.phone,
                        deliveryman.workAddress
                    )
                    val intent = Intent(this@CustomerLoginActivity, MainActivity::class.java)
                    intent.putExtra("status", 1);
                    startActivity(intent)
                    finish()
                } else {
                    Toast.makeText(this@CustomerLoginActivity, msg, Toast.LENGTH_SHORT).show()
                    password_et.setText("")
                }
            }
        }
